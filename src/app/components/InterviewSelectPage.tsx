...
const goToMcq = () => {
  if (!jobDescription) {
    ...
    if (backup) {
      navigate("/interview/mcq", {
        state: { mode: "mcq", jobDescription: backup },
      });
      return;
    }
  ...
  navigate("/interview/mcq", {
    state: { mode: "mcq", jobDescription: jobDescription },
  });
};
  const goToEssay = () => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <button
            ...
            className={`group text-left bg-white dark:bg-[#1a1d27] border-2 rounded-2xl p-6 transition-all duration-200 shadow-sm hover:shadow-md ${
              !jobDescription
                ? "border-gray-300 dark:border-gray-700 opacity-50 cursor-not-allowed"
                : "border-gray-200 dark:border-gray-700/60 hover:border-blue-400 dark:hover:border-blue-500"
            }`}
          >
