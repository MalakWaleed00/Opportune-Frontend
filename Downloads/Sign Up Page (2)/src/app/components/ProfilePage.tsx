...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f1117] transition-colors duration-300">
      {/* Cover */}
      <div className="h-40 bg-gradient-to-r from-gray-800 to-gray-600 relative">
        {editing && (
          <div className="absolute top-4 right-4 flex gap-2">
            <button onClick={cancel} className="flex items-center gap-1.5 bg-white text-gray-700 border border-gray-200 rounded-lg px-3 py-1.5 text-sm font-medium hover:bg-gray-50">
              <X size={13} /> Cancel
            </button>
            <button onClick={save} className="flex items-center gap-1.5 bg-black text-white rounded-lg px-3 py-1.5 text-sm font-medium hover:bg-gray-800">
              <Check size={13} /> Save
            </button>
          </div>
        )}
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="flex items-end justify-between -mt-12 mb-4">
          <div className="relative">
            <div className="w-24 h-24 rounded-full border-4 border-white bg-gray-200 overflow-hidden flex items-center justify-center shadow-md">
              {cur.pp
          ...
          <div className="flex gap-2 mt-14">
            <a href={profile.cvLink} target="_blank" rel="noreferrer"
              className="flex items-center gap-2 border border-gray-300 rounded-full px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
              <Download size={14} /> Resume
            </a>
            {!editing && (
              <button onClick={startEdit}
                className="flex items-center gap-2 bg-black text-white rounded-full px-4 py-2 text-sm font-semibold hover:bg-gray-800 transition-colors">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {editing && (
                  <input value={skillInput} onChange={e => setSkillInput(e.target.value)} onKeyDown={addSkill}
                    placeholder="Add…" className="text-sm border border-dashed border-gray-300 rounded-full px-3 py-1 outline-none focus:border-black w-24" />
                )}
              </div>
            </Section>
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <Section label="Experience">
              <div className="space-y-3">
                {cur.experience.map((exp, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 border border-gray-100 rounded-xl bg-white hover:bg-gray-50 transition-colors">
                    <div className="w-9 h-9 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0">
                ...
                {editing && (
                  <button onClick={addExp} className="w-full border border-dashed border-gray-300 rounded-xl py-2 text-xs text-gray-400 hover:border-black hover:text-black transition-colors">
                    + Add Experience
                  </button>
                )}
              </div>
            ...
            <Section label="CV / Resume">
              {editing
                ...
                : <a href={profile.cvLink} target="_blank" rel="noreferrer" className="text-sm text-gray-400 hover:text-white transition-colors break-all">{profile.cvLink}</a>}
            </Section>
          </div>
        </div>
      </div>
...
const inp = "border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#1a1d27] text-gray-900 dark:text-white rounded-lg px-2.5 py-1.5 text-sm outline-none focus:border-black dark:focus:border-gray-500 transition-colors";
